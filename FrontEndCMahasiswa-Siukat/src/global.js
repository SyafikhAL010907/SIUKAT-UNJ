import swal from 'sweetalert';
import moment from 'moment';
import Cookies from 'universal-cookie';

const _cookies = new Cookies();

const getEnv = (name, fallback) => {
    try {
        if (typeof process !== 'undefined' && process.env && process.env[name]) {
            return process.env[name];
        }
    } catch (e) {}
    return fallback;
};

const cookieName = getEnv('REACT_APP_COOKIE_NAME', "siukat_cookie_2026");

// Wrap cookies so .get(cookieName) always falls back to localStorage
// Wrap cookies so .get(cookieName) always falls back to sessionStorage
const cookies = new Proxy(_cookies, {
    get(target, prop) {
        if (prop === 'get') {
            return function (name, ...args) {
                // Prioritaskan sessionStorage untuk token login agar otomatis logout saat tab ditutup
                if (name === cookieName && typeof sessionStorage !== 'undefined') {
                    const ssVal = sessionStorage.getItem('siukat_token');
                    // Jika di sessionStorage kosong atau berisi string "undefined", paksa hapus cookie (Logout)
                    if (!ssVal || ssVal === 'undefined' || ssVal === 'null') {
                        if (target.get(name)) target.remove(name, { path: '/' });
                        return undefined;
                    }
                    // Pastikan cookie tetap sinkron (untuk backend/request)
                    if (target.get(name) !== ssVal) {
                        target.set(name, ssVal, { path: '/' });
                    }
                    return ssVal;
                }
                return target.get(name, ...args);
            };
        }
        const value = target[prop];
        return typeof value === 'function' ? value.bind(target) : value;
    }
});

let apiBase = getEnv('REACT_APP_API_URL', 'http://10.255.1.149:8080/api/v1/');
// let apiBase = getEnv('REACT_APP_API_URL', 'https://10.255.1.149:8080/api/v1/');

// Smart Switch: Jika dibuka via localhost, otomatis arahkan API ke localhost juga
if (typeof window !== 'undefined' && (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')) {
    if (apiBase.includes('10.255.1.149')) {
        apiBase = apiBase.replace('10.255.1.149', 'localhost');
    }
}
const service = apiBase;

// const service = 'http://192.168.4.174:3001'
const storage = service;
const baseUrl = service.replace('/api/v1', '');

const errorIcon = 'data:image/png;base64,';
const infoIcon = 'data:image/png;base64,';
const successIcon = 'data:image/png;base64,';

const errLog = (err) => {
    if (err !== undefined && err !== null) {
        // 1. Check for standard Axios error response with data
        if (err.response?.data) {
            const data = err.response.data;
            if (typeof data === "string" && data !== "") return data;
            if (typeof data === "object") {
                 // Try to find common error keys
                 return data.message || data.error || data.msg || JSON.stringify(data);
            }
        }
        
        // 2. Check for standard Error object message
        if (err.message) return err.message;
        
        // 3. Fallback to constant string if it's just a string
        if (typeof err === "string") return err;
        
        // 4. Fallback to status
        if (err.status) return `Error: ${err.status} ${err.statusText || ""}`;
        
        return "Terjadi kesalahan sistem yang tidak diketahui";
    } else {
        return "Koneksi terputus atau data tidak ditemukan";
    }
};

const notif = function (title, msg, type) {
    swal({
        title: title,
        text: msg,
        icon: type,
        button: false,
        timer: 3000
    });
};

const rupiah = (angka) => {
    if (angka !== undefined) {
        var value = angka.toString().replace('.', '');
        var rupiah = '';
        var angkarev = value.toString().split('').reverse().join('');
        for (var i = 0; i < angkarev.length; i++) if (i % 3 === 0) rupiah += angkarev.substr(i, 3) + '.';
        return 'Rp ' + rupiah.split('', rupiah.length - 1).reverse().join('');
    }
};

const dateConverter = (date) => {
    if (!date || date === "0000-00-00" || date === "" || date === "0001-01-01T00:00:00Z") return "-"
    const birthdate = moment(date)
    return birthdate.isValid() ? birthdate.format("DD MMMM YYYY") : "-"
};

const getToken = () => {
    if (typeof sessionStorage !== 'undefined') {
        const token = sessionStorage.getItem('siukat_token');
        if (token === 'undefined' || token === 'null' || !token) return undefined;
        return token;
    }
    const cookieToken = cookies.get(cookieName);
    return (cookieToken === 'undefined' || cookieToken === 'null') ? undefined : cookieToken;
};

const setToken = (token) => {
    if (typeof sessionStorage !== 'undefined') {
        sessionStorage.setItem('siukat_token', token);
    }
    cookies.set(cookieName, token, { path: '/' });
};

const removeToken = () => {
    if (typeof sessionStorage !== 'undefined') {
        sessionStorage.removeItem('siukat_token');
    }
    cookies.remove(cookieName, { path: '/' });
};

export {
    cookies,
    service,
    storage,
    baseUrl,
    errorIcon,
    infoIcon,
    successIcon,
    notif,
    cookieName,
    rupiah,
    errLog,
    dateConverter,
    getToken,
    setToken,
    removeToken,
};