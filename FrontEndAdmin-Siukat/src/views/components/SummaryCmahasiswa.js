...
class SummaryCmahasiswa extends React.Component {
     ...
     */
    renderCard(title, count, percentage, smallText, icon, colorClass, barColor, textColor = "text-white") {
        ...
        return (
            <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-sm border border-gray-100 p-6 transition-all hover:shadow-xl hover:-translate-y-1.5 group">
                <div className="flex justify-between items-start mb-5">
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-[0.1em] text-gray-400 mb-1">{title}</p>
                        <h3 className="text-3xl font-black text-slate-800 tracking-tight">
                            {count || 0}
                        </h3>
                    </div>
                    <div className={`p-3.5 rounded-2xl ${colorClass} ${textColor} shadow-lg transition-transform group-hover:scale-110 group-hover:rotate-6`}>
                ...
                <div className="space-y-3">
                    <div className="flex justify-between items-center text-[11px]">
                        <div className="flex items-center space-x-1.5">
                            <span className="font-bold text-slate-700">{safePercentage}%</span>
                            <span className="text-gray-400 font-semibold uppercase tracking-wider opacity-60">Progres</span>
                        </div>
                        <span className="text-gray-400 font-medium italic">{smallText}</span>
                    </div>
