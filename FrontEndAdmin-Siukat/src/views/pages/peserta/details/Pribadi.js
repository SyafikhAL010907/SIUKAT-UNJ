...
let FormMahasiswa = (props) => {
  ...
  return (
    <Modal
      isOpen={toggleMahasiswa}
      toggle={handleToggleMahasiswa}
      size="lg"
      centered
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div>
              ...
              >
                ...
                {ref_provinsi.map((v, i) => (
                  <option key={i} value={v.provinsi_id}>
                    {v.provinsi_nama}
                  </option>
                ))}
              </Field>
            ...
            <div>
              ...
              >
                ...
                {ref_kabkot.map((v, i) => (
                  <option key={i} value={v.id || v.kab_id}>
                    {v.nama || v.kab_nama}
                  </option>
                ))}
              </Field>
            ...
            <div className="md:col-span-2">
              ...
              >
                ...
                {ref_kecamatan.map((v, i) => (
                  <option key={i} value={v.id || v.kecam_id}>
                    {v.nama || v.kecam_nama}
                  </option>
                ))}
              </Field>
  ...
  initLoad = async () => {
    ...
    try {
      const res = await dispatch(
        cmahasiswa.fetchAllData(token, noPeserta, this.props.atribut),
      );
      const val = res?.value?.data || res?.payload?.data;

      if (val) {
        if (val.provinsi_cmahasiswa)
          dispatch(kabkot.fetchForCmahasiswa(val.provinsi_cmahasiswa));
        if (val.kabkot_cmahasiswa)
          dispatch(kecamatan.fetchForCmahasiswa(val.kabkot_cmahasiswa));
      }
    } catch (e) {
  ...
  submitMahasiswa = async (values) => {
    ...
    try {
      await this.props.dispatch(
        cmahasiswa.updateData(token, formData, this.props.noPeserta),
      );
      await this.props.dispatch(
        cmahasiswa.fetchAllData(
          token,
        ...
        {isModeSanggah && (
          <div className="mb-6 p-4 bg-orange-50 border border-orange-200 rounded-2xl flex items-center text-orange-700 animate-pulse">
            ...
            <span className="text-sm font-bold uppercase">
              Mode Sanggah Aktif: Anda dapat mengubah data profil sekarang.
            </span>
          </div>
        )}
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
              ...
              <InfoItem
                ...
                value={`${data.tempat_lahir_cmahasiswa || "-"}, ${dateConverter(data.tanggal_lahir_cmahasiswa)}`}
              />
              <InfoItem label="Golongan Darah" value={data.goldar_cmahasiswa} />
              <InfoItem label="Nomor Telepon" value={data.telepon_cmahasiswa} />
              <InfoItem
                label="Penghasilan"
                value={`${rupiah(data.penghasilan_cmahasiswa)} / bulan`}
              />
