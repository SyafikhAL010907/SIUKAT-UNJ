              ...
              <FormGroup row>
                ...
                <Col md={5} xs={12}>
                  <Field
                    ...
                    name="sampingan_ibu"
                    id="sampingan_ibu"
                    placeholder="Penghasilan Sampingan"
                  />
                </Col>
                <Col md={4} xs={12}>
                  <Alert color="success" className="py-2 text-sm">
                    {rupiah(sampingan_ibu || 0)}
                  </Alert>
                </Col>
              </FormGroup>
              <FormGroup row>
...
class Ibu extends React.Component {
  ...
  fetchInitialData = () => {
    ...
    this.props.dispatch(
      ibu.fetchAllData(token, this.props.noPeserta, this.props.atribut),
    );

    if (this.props.ibu) {
      if (this.props.ibu.provinsi_ibu)
        this.props.dispatch(kabkot.fetchForIbu(this.props.ibu.provinsi_ibu));
      if (this.props.ibu.kabkot_ibu)
        this.props.dispatch(kecamatan.fetchForIbu(this.props.ibu.kabkot_ibu));
    }
  };
  ...
  submitIbu = (values) => {
    ...
    return this.props
      .dispatch(ibu.updateData(token, formData, this.props.noPeserta))
      .then((res) => {
        ...
        {isModeSanggah && (
          <div className="mb-4 p-4 bg-orange-50 border border-orange-200 rounded-2xl flex items-center text-orange-700 animate-pulse">
            ...
            <span className="text-sm font-bold uppercase">
              Mode Sanggah Aktif: Anda dapat mengubah data ibu sekarang.
            </span>
          </div>
        )}
        ...
        <div className="flex justify-between items-end border-b border-gray-200 pb-4">
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Informasi orang tua kandung / ibu.
            </p>
          </div>
          {isModeSanggah && (
            <button
                ...
                <>
                  ...
                  <tr>
                    ...
                    <td className="p-4">
                      {data.scan_ktp_ibu ? (
                        <a
                          href={storage + "/" + data.scan_ktp_ibu}
                          target="_blank"
                          rel="noopener noreferrer"
                    ...
                    <td className="p-4 text-gray-700">
                      {data.tempat_lahir_ibu || data.tanggal_lahir_ibu ? (
                        <>
                          ...
                          {dateConverter(data.tanggal_lahir_ibu)}
                        </>
                      ) : (
                        "-"
                      )}
                  ...
                  <tr>
                    ...
                    <td className="p-4">
                      {data.scan_slip_ibu ? (
                        <a
                          href={storage + "/" + data.scan_slip_ibu}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 font-bold text-sm"
                        >
...
FormIbu = connect(
  (store) => {
    let { status_ibu, penghasilan_ibu, sampingan_ibu } = selector(
      store,
      "status_ibu",
      "penghasilan_ibu",
      "sampingan_ibu",
