...
class ProgramStudi extends React.Component {
    ...
    this.setState({ [field]: value }, () => {
      ...
      if (selectedYear) {
        if (selectedYear === "2026") {
          this.props.dispatch(
            rekapitulasi.fetchDataProdi(token, selectedYear, selectedJalur),
          );
        } else {
  ...
  handleExport = () => {
    if (!this.state.selectedYear) {
      return swal(
        "Peringatan!",
        "Pilih Tahun dulu broo sebelum export!",
        "info",
      );
    }
    ...
    if (!rekapProdi.length) {
      return swal(
        "Waduh!",
        "Data rekapitulasi masih kosong, gak ada yang bisa diexport broo!",
        "warning",
      );
    }
        ...
        <Container fluid className="px-2 md:px-12 pt-5 md:pt-10">
            ...
            <Row className="align-items-center">
              ...
              <Col lg="7" md="12" className="mt-4 lg:mt-0">
                <div className="flex flex-col lg:flex-row gap-4 items-center justify-center lg:justify-end font-['Montserrat'] w-full">
                  {/* --- 1. Dual Filter (Pill Style) --- */}                  <div className="flex flex-col sm:flex-row w-full lg:w-auto bg-white rounded-2xl shadow-lg border border-emerald-50 p-1.5 sm:p-1 gap-1 sm:gap-0">
                    <select
          ...
          <div className="bg-white border border-emerald-500/10 rounded-[20px] md:rounded-[32px] shadow-lg overflow-hidden pt-6 md:pt-[50px]">
            <Table
              ...
              hover
              borderless
              className="premium-table mb-0 w-full"
            >
              <thead>
                <tr className="bg-emerald-500">
                  ...
                  <th
                    colSpan="8"
                    className="bg-emerald-600/20 text-white font-[900] uppercase text-[10.5px] tracking-[0.12em] py-2 px-[15px] border-none shadow-[0_2px_4px_rgba(0,0,0,0.1)] text-center"
                  >
                ...
                <tr className="bg-emerald-500">
                  {GOLONGAN_LIST.map((gol) => (
                    <th
                      key={gol}
                      className="bg-emerald-500 text-white font-[900] uppercase text-[10.5px] tracking-[0.12em] py-2 border-none shadow-[0_2px_4px_rgba(0,0,0,0.1)] text-center border-l border-white/5"
                    >
                      {gol}
                    </th>
                  ))}
                  ...
                  filteredData.map((item, idx) => (
                    ...
                    >
                      ...
                      <td className="text-center py-5">
                        <span className="bg-[#ecfdf5] color-[#059669] px-3.5 py-2 rounded-xl font-black border-2 border-[#d1fae5]">
                      ...
                      {GOLONGAN_LIST.map((gol) => (
                        <td
                          key={gol}
                          className="text-center py-5 text-slate-500 font-black"
                        >
                          {item[gol] || 0}
                        </td>
                      ))}
                      <td className="text-center py-5 font-black text-sky-600">
                        {item.bidikmisi || 0}
                      </td>
              ...
              <tfoot>
                <tr className="bg-emerald-600 text-white border-none">
                  ...
                  <td className="text-center p-[25px_15px] font-[900] uppercase text-[11px] border-none">
                    <div className="bg-yellow-400 text-emerald-950 px-3 py-1.5 rounded-lg inline-block">
                      {this.calculateGrandTotal("total_mahasiswa")}
                    </div>
                  </td>
                  {GOLONGAN_LIST.map((gol) => (
                    <td key={gol} className="text-center opacity-60 italic">
                      {this.calculateGrandTotal(gol)}
                    </td>
                  ))}
                  <td className="text-center text-sky-300">
                    {this.calculateGrandTotal("bidikmisi")}
                  </td>
                  <td className="text-center text-amber-300">
                    {this.calculateGrandTotal("subtotal")}
                  </td>
      ...
      <div className="p-12 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-emerald-700/60 font-medium leading-relaxed px-5">
            Data rekapitulasi untuk periode yang Anda pilih sedang dalam tahap
            sinkronisasi server. Mohon tunggu update selanjutnya bro!
          </p>
        </div>
