...
export const exportRekapExcel = async (type, data, totals) => {
    ...
    const fileName = isFakultas ? `Rekap_Fakultas_${new Date().toLocaleDateString()}.xlsx` : `Rekap_Prodi_${new Date().toLocaleDateString()}.xlsx`;
    ...
    headerRow1.values = ['NO', labelColumn, 'TOTAL MHS', 'DISTRIBUSI GOLONGAN UKT', '', '', '', '', '', '', '', 'KIPK', 'SELESAI', 'TOTAL NOMINAL UKT'];
    ...
    data.forEach((item, index) => {
        const row = worksheet.addRow([
            ...
            item.I || 0, item.II || 0, item.III || 0, item.IV || 0,
            item.V || 0, item.VI || 0, item.VII || 0, item.VIII || 0,
            item.bidikmisi || 0,
            item.subtotal || 0,
            item.total_ukt || 0
        ]);
    ...
    if (totals) {
        const footerRow = worksheet.addRow([
            ...
            totals.I, totals.II, totals.III, totals.IV,
            totals.V, totals.VI, totals.VII, totals.VIII,
            totals.bidikmisi,
            totals.subtotal,
            totals.total_ukt
        ]);
        ...
        footerRow.eachCell((cell, colNumber) => {
            cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF065F46' } }; // Darker green
            cell.font = BOLD_WHITE_FONT;
            cell.border = BORDER_STYLE;
            cell.alignment = CENTER_ALIGN;
            if (colNumber === 14) formatCurrency(cell);
...
export const exportMasterDataExcel = async (data) => {
    console.log('[SIUKAT] Memulai Export Master Data (Sync with PDF)...', { dataCount: data.length });
    try {
        const workbook = new ExcelJS.Workbook();
    ...
    data.forEach((item, idx) => {
        ...
        const row = worksheet.addRow({
            ...
            bidik_misi_cmahasiswa: item.bidik_misi_cmahasiswa === 'Ya' ? 'Ya' : 'Tidak',
            fakultas_nama: item.fakultas ? item.fakultas.nama : '-',
            prodi_nama: item.prodi ? item.prodi.nama : '-',
            golongan_label: isSanggah ? `Sanggah: ${item.golongan_id}` : item.golongan_id,
        ...
        row.eachCell((cell, colNumber) => {
            cell.border = BORDER_STYLE;
            cell.alignment = { vertical: 'middle', horizontal: colNumber <= 3 ? 'center' : (colNumber >= 8 && colNumber <= 9 ? 'right' : 'left') };
    ...
    const buffer = await workbook.xlsx.writeBuffer();
    saveAs(new Blob([buffer]), `Data_Master_Siukat_${new Date().toLocaleDateString()}.xlsx`);
